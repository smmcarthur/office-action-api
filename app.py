...
def fetch_office_action():
    ...
    if response.status_code != 200:
        return jsonify({'error': f'Failed to fetch data for serial {serial_number}'}), 500
    ...
    if not office_actions:
        return jsonify({'error': 'No office action found'}), 404
    ...
    if pdf_response.status_code != 200:
        return jsonify({'error': 'Failed to download Office Action PDF'}), 500
    ...
    return jsonify({
        ...
        'office_action_text': text[:5000]  # Limit to 5000 chars for safety
    })
